...
json_doc = JSONLoader(file_path="sample-data/faq.json",
                       jq_schema=".faqs[]", 
                       text_content=False)
faq_docs = json_doc.load()

csv_doc = CSVLoader(file_path="sample-data/products.csv")
...
order_df = pd.read_csv("sample-data/order_data_50.csv")
